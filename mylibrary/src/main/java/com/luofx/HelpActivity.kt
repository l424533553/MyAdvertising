...
/**
 *
 * 帮助内容 ，将 word文件转成Html文件，然后用web打开文档显示
 * 注意點:將word中的图片保存在了本地  ，然后 根据相对地址，打开html网页上 加载图片
 */
class HelpActivity : AppCompatActivity() {
    //    //SD卡在设备的目录为/mnt/sdcard，并不存在/mnt/asec/目录
    //    private var docPath = "/mnt/sdcard/"
    //文件名称  // 文件路径放在了 main/assets路径下   .doc文件需要是 2003 的格式，否则太高级读不出来
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!isNotFirst) {//第一次
            val sysApplication = application as MyBaseApplication
